...
const EventDetailPage = () => {
  ...
  // Загружаем событие из WordPress или используем базовое событие
  useEffect(() => {
    const fetchEvent = async () => {
      try {
        setLoading(true);
        const eventData = await wordpressApi.getEventById(eventId);

        if (eventData) {
          // Создаем объект события с галереей и этапами
          const eventWithGallery = {
            ...eventData,
            gallery: [
              { image: eventData.featuredImage || image1, title: 'Основное изображение', description: eventData.title },
              { image: image1, title: 'Дополнительное фото 1', description: 'Дополнительное изображение события' },
              { image: image1, title: 'Дополнительное фото 2', description: 'Дополнительное изображение события' },
              { image: image1, title: 'Дополнительное фото 3', description: 'Дополнительное изображение события' },
              { image: image1, title: 'Дополнительное фото 4', description: 'Дополнительное изображение события' },
              { image: image1, title: 'Дополнительное фото 5', description: 'Дополнительное изображение события' },
              { image: image1, title: 'Дополнительное фото 6', description: 'Дополнительное изображение события' },
              { image: image1, title: 'Дополнительное фото 7', description: 'Дополнительное изображение события' },
              { image: image1, title: 'Дополнительное фото 8', description: 'Дополнительное изображение события' },
              { image: image1, title: 'Дополнительное фото 9', description: 'Дополнительное изображение события' }
            ],
            stages: [
              'Подготовка и планирование мероприятия.',
              'Проведение основного этапа.',
              'Завершение и подведение итогов.'
            ],
            results: [
              'Мероприятие успешно проведено.',
              'Достигнуты поставленные цели.',
              'Получена положительная обратная связь.'
            ]
          };
          setEvent(eventWithGallery);
        } else {
          // Базовое событие при ошибке
          const fallbackEvent = {
            id: eventId,
